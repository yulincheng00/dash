...
def update(startDate, endDate, freqs, ip):
    # get chart
    # area_fig = area.update(startDate, endDate, 'rule.level', freqs, 'Alert level evolution',ip)
    ...
    # # 若有資料
    # donut_mitre_fig = donut.update(startDate, endDate, 'rule.mitre.technique', mitre_title, mitre_topNum, ip)
    # donut_mitre_fig.update_layout(legend=dict(x=1.2)) # legend 會擋到 label, 故往右移
    # donut_agent_fig = donut.update(startDate, endDate, 'agent.name', donut_agent_title, agent_topNum, ip)
    # bar_agent_fig = bar.se_update(startDate, endDate, freqs, 'agent.name', bar_agent_title, agent_topNum, ip)
    ...
    total = 2#posts.count_documents({'$and':[{'timestamp': {"$gte":startDate}},
    #                                        {'timestamp': {"$lte":endDate}},
    #                                        {'rule.id':{"$eq":id}}]})

    high = 2 #posts.count_documents({'$and':[{'timestamp': {"$gte":startDate}},
    #                                          {'timestamp': {"$lte":endDate}},
    #                                          {'rule.id':{"$eq":id}},
    #                                          {'rule.level': {"$gte":12}}]})
